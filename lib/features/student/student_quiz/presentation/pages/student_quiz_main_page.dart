...
class StudentQuizMainPage extends StatelessWidget {
  ...
  SliverWidget _buildSliverWidget(BuildContext context, Quiz quiz) {
    final coursesModel = context.read<SubscribeStudentCoursesCubit>().courseSelected;
  ...
  Widget _buildQuizTimerCard(BuildContext context, Quiz quiz) {
    ...
    Widget getTimerContent() {
      if (!quiz.endIn.isBefore(now) && quiz.startIn.isBefore(now)) {
        return _buildTimerRow('Remaining to end Quiz', quiz.endIn.difference(now), Colors.red);
      } else if (!quiz.endIn.isBefore(now) && quiz.startIn.difference(now).inDays < 5) {
        return _buildTimerRow('Remaining to start Quiz', quiz.startIn.difference(now), Colors.blue);
    ...
    return Column(
      children: [
        _buildListTile(
          ...
          subTitle: '${quiz.startIn.day}/${quiz.startIn.month}/${quiz.startIn.year}',
  ...
  Widget _buildStartQuizButton(BuildContext context, Quiz quiz) {
    if (!quiz.endIn.isBefore(DateTime.now()) && quiz.startIn.isBefore(DateTime.now())) {
      return Column(
        children: [
          verticalSpace(24),
          SizedBox(
  ...
  ListTile _buildListTile({required String title, required String subTitle, required String svgUrl}) {
    return ListTile(
