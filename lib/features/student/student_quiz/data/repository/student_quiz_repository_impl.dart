...
class StudentQuizRepositoryImpl implements StudentQuizRepository {
  ...
  @override
  Future<Either<Failure, List<Quiz>>> getAllStudentQuiz({required int idCourse})async {
    if(await networkInfo.isConnected){
      try{
        final String token = await SharedPrefHelper.getString(SharedPrefKeys.cachedToken);
        final listQuiz = await remoteDataSourceImpl.getAllStudentQuiz(idCourse: idCourse, token: token);
  ...
  @override
  Future<Either<Failure, List<QuestionStudentQuiz>>> getAttemptStudentQuiz({required int idQuiz, required int idCourse}) async{
    if(await networkInfo.isConnected){
      try{
        final String token = await SharedPrefHelper.getString(SharedPrefKeys.cachedToken);
        final listQuestionStudentQuiz = await remoteDataSourceImpl.getAttemptStudentQuiz(idQuiz:idQuiz ,idCourse: idCourse, token: token);
  ...
  @override
  Future<Either<Failure, Unit>> submitAnswerQuiz({required int idQuiz, required int idCourse, required Map data})async {
    if(await networkInfo.isConnected){
      try{
        final String token = await SharedPrefHelper.getString(SharedPrefKeys.cachedToken);
         await remoteDataSourceImpl.submitAnswerQuiz(data: data,idQuiz:idQuiz ,idCourse: idCourse, token: token);
        return Right(unit);
      }on ServerException{
        return Left(ServerFailure());
      }
