...
class EditAdvertisementsInfoDialog extends StatelessWidget {
  Ads ads;
  int index;
   EditAdvertisementsInfoDialog({super.key,required this.ads,required this.index});

  @override
  Widget build(BuildContext context) {
    context.read<CourseMainCubit>().adsTextEditionController = TextEditingController(text: ads.text);
    context.read<CourseMainCubit>().colorSelectedTextField = ads.colorText;
    return AlertDialog(
                ...
                child: AppTextButton(
                  onPressed: () {
                    if(context.read<CourseMainCubit>().globalAdsKey.currentState!.validate()){
                      context.read<CourseMainCubit>().emitEditAds(index: index);
                      context.pop();
                    }
